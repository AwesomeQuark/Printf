...
/*   Updated: 2019/01/07 16:26:02 by conoel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_itoabu(unsigned long long nb, t_flag *all)
{
	...
	while (all->minus == 1 && --all->minsize >= size)
		all->buffer[all->buffer_index + i--] = all->zero &&
all->precision < 0 ? '0' : ' ';
	nb == 0 && (all->precision != 0 || all->hash != 0) ?
all->buffer[all->buffer_index] = '0' : 0;
	while (nb > 0 || j > 0)
	{
		if (nb % base <= 9)
			all->buffer[all->buffer_index + i--] = nb % base + '0';
		else
			all->buffer[all->buffer_index + i--] = nb % base +
(all->type == 'X' ? 'A' : 'a') - 10;
		nb /= base;
		j--;
	}
	all->type == 'o' && all->hash == 1 && all->zero == 0 ?
all->buffer[all->buffer_index + i--] = '0' : 0;
	(all->type == 'X' || all->type == 'x') && all->hash == 1 &&
all->zero == 0 ? all->buffer[all->buffer_index + i--] = all->type : 0;
	(all->type == 'X' || all->type == 'x') && all->hash == 1 &&
all->zero == 0 ? all->buffer[all->buffer_index + i--] = '0' : 0;
	while (all->minus == 0 && --all->minsize >= size)
		all->buffer[all->buffer_index + i--] = all->zero &&
all->precision < 0 ? '0' : ' ';
	all->type == 'o' && all->hash == 1 && all->zero == 1 ?
all->buffer[all->buffer_index + i--] = '0' : 0;
	(all->type == 'X' || all->type == 'x') && all->hash == 1 &&
all->zero == 1 ? all->buffer[all->buffer_index + i--] = all->type : 0;
	(all->type == 'X' || all->type == 'x') && all->hash == 1 &&
all->zero == 1 ? all->buffer[all->buffer_index + i--] = '0' : 0;
